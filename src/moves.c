...
/*   Updated: 2019/11/03 19:48:53 by chermist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_to_end(t_support *sup, t_vec *path)
{
	...
	if (sup->opt.visu)
	{
		sup->moves = ft_vnew(50, sizeof(t_vec*));
...
void	print_moves(t_vec *p, t_support *sup)
{
	...
	if (!(i[0] = 0) && sup->opt.visu)
		sup->moves = ft_vnew(500, sizeof(t_vec*));
	while (i[0] < sup->ants && (i[1] = -1))
	{
		if (sup->opt.visu && (move = ft_vnew(500, sizeof(t_move*))))
